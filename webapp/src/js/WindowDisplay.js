...
const fixInstallmentValue = (installmentsString) => {
  ...
  const formattedMoney = `R$ ${moneyString.substring(0, dotIndex)},${moneyString.substring(dotIndex + 1, moneyString.length)}`

  return `${installmentsString.substr(0, blankIndex)}${formattedMoney}${installmentsString.substr(tagIndex, installmentsString.length)}`;
}
...
const generateReccomended = (reccomendedItems) => {
  ...
  reccomendedItems.forEach(reccomendedItem => {
    ...
    if (reccomendedItem.oldPrice) {
      ...
      const oldPriceNode = document.createTextNode(`De: ${reccomendedItem.oldPrice}`);
    ...
    slidePrice.classList.add('red-font', 'block-display', 'big-font', 'bold-font');
    ...
    slideInstallments.innerHTML = fixInstallmentValue(reccomendedItem.productInfo.paymentConditions);
...
const generateVisited = (visitedItem) => {
  const visitedItemPriceNode = document.createTextNode(`Por: ${visitedItem.price}`);
  const visitedItemOldPriceNode = document.createTextNode(`De: ${visitedItem.oldPrice}`);
  ...
  $('#visitedItemInstallments').innerHTML = fixInstallmentValue(visitedItem.productInfo.paymentConditions);
}
